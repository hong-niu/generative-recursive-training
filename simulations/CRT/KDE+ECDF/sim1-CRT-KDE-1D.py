...
We study a recursive kernel density estimator (KDE) where, at each 
iteration n:
...
def sample_true_np(n, rng):
    """Draw n i.i.d. samples from the two-component Gaussian mixture (NumPy)."""
    if n <= 0:
...
def true_pdf_np(x):
    ...
    return w1 * norm.pdf(x, mu1, sigma1) + (1.0 - w1) * norm.pdf(x, mu2, sigma2)


def true_cdf_np(x):
    """Ground-truth mixture CDF evaluated at x (NumPy)."""
    return w1 * ndtr((x - mu1) / sigma1) + (1.0 - w1) * ndtr((x - mu2) / sigma2)
...
# Fixed evaluation grid for deterministic W1 and MMD computation
...
DTYPE = torch.float64  # unchanged precision choice
...
# Precompute Gaussian kernel matrix for MMD² on the grid
MMD_SIGMA = 0.5
Xgrid = xg_t.view(-1, 1)
...
# ============================================================
# 2. Distributional metrics and numerical integration
# ============================================================
def silverman_bw_np(x):
    """Silverman's rule-of-thumb bandwidth (NumPy)."""
    s = np.std(x, ddof=1)
    n = len(x)
    if n <= 1 or s == 0:
        return 1.0
...
def cdf_from_pdf_on_grid_torch(pdf_vals, dx, renormalize=True):
    """
    Compute a CDF from density values on a uniform grid by trapezoidal integration.
    """
...
def empirical_cdf_on_grid_torch(samples, xg_t):
    """Empirical CDF evaluated on xg_t: F_hat(x) = (1/n) sum 1{X_i ≤ x}."""
...
def mmd2_from_pdfs_torch(f_true, f_model, K, dx):
    """
    Compute MMD² between two densities on a fixed grid using a Gaussian kernel.
    """
...
# ============================================================
# 3. Recursive density estimation procedure
# ============================================================
def run_recursive_estimator(
    p_target=0.5,
    alpha=0.5,  # target real fraction
    m1=50,
    T=200,
    use_ecdf=False,
    h0=0.5,
    ...
    """
    Run the recursive density estimation procedure.

    Returns a dict with time series for M, W1, MMD², bandwidths, and histories.
    """
    device = DEVICE
    dtype = DTYPE

    # Determine m2 so that alpha ≈ m1 / (m1 + m2)
    if alpha >= 1.0:
        m2 = 0
        alpha = 1.0
    elif alpha <= 0.0:
    ...
    # Effective realized real fraction (due to integer rounding)
    ...
    with torch.no_grad():
        for n in range(1, T + 1):
            ...
            # (3) Density/CDF estimation + synthetic generation
            if use_ecdf:
    ...
    return {
        ...
        "p_target": p_target,
        "alpha": float(alpha),          # requested real fraction
        "alpha_eff": float(alpha_eff),  # realized fraction due to rounding
        "m1": m1,
        "m2": m2,
        "use_ecdf": use_ecdf,
...
# ============================================================
# 4. Power-law exponent estimation
# ============================================================
def estimate_power_law_slope(M, vals, drop_first=10):
    """
    Fit log(vals) = a + b log(M). Returns slope b (i.e., vals ~ M^b).
    """
    M = np.asarray(M)
...
if __name__ == "__main__":
    ...
    def plot_with_trend(M, values, metric_name, p_val, alpha_val, drop_first=10, save_path=None):
        """
        Plot log10(values) vs log10(M) and optionally overlay a fitted linear trend line.
        """
        eps = 1e-20
        log_M = np.log10(M)
    ...
    for alpha in alphas:
        ...
        for p in p_values:
            ...
            for rep in range(n_reps):
                ...
                W1_vals = out["W1"]
                MMD2s = out["MMD2"]
                MMDs = np.sqrt(np.maximum(MMD2s, 0.0))
                ...
                log(
                    f"Rep {rep + 1}: slope_W1={slope_W1:.3f}, "
                    f"slope_MMD={slope_MMD:.3f}, "
                    f"slope_MMD2={slope_MMD2:.3f}"
                )
            ...
            slopes_MMD = np.array(slopes_MMD)
            slopes_MMD2 = np.array(slopes_MMD2)

            mean_W1, std_W1 = slopes_W1.mean(), slopes_W1.std(ddof=1)
            mean_MMD, std_MMD = slopes_MMD.mean(), slopes_MMD.std(ddof=1)
            ...
            if np.isclose(p, alpha, atol=1e-8):
                log("Critical regime: metrics were normalized by log(M_n) or log(M_n)^2 before slope fitting.")

            # Scaling plots for the last repetition only
            M_vals = last_out["M"]
            W1_vals = last_out["W1"]
            MMD2s = last_out["MMD2"]
            MMDs = np.sqrt(np.maximum(MMD2s, 0.0))
            ...
            plot_with_trend(
                ...
                alpha,
                drop_first=drop_first,
                save_path=fig_dir / f"{run_label}_MMD.png",
            )
    ...
    def plot_alpha_vs_mean_slope(
        ...
        metric_label_tex = r"W_1" if metric_label == "W1" else metric_label
        ...
        for p in unique_ps:
            ...
            # Plot convention: y-axis shows the estimated positive exponent magnitude
            vals_plot = -means

            if use_error_bars:
                stds = np.array(
                    [np.array(r[slopes_key]).std(ddof=1) * std_factor for r in subset],
                    dtype=float,
                )
                stds = stds[order]
        ...
        # Fix y-axis limits for comparability across metrics
        plt.ylim(0.0, 0.65)
